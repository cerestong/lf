...
static double strtod_int(const char *, char **, int *, char *, size_t);
static char *dtoa(double, int, int, int *, int *, char **, char *, size_t);
static void dtoa_free(char *, char *, size_t);

size_t mydtoa::fcvt(double x, int precision, char *to, bool *error)
{
  ...
  if (precision > 0)
  {
    ...
    for (i = precision - std::max(0, (len - decpt)); i > 0; i--)
      *dst++ = '0';
  }
...
size_t mydtoa::gcvt(double x, gcvt_arg_type type, int width, char *to, bool *error)
{
  ...
  res = dtoa(x, 4, type == GCVT_ARG_DOUBLE ? width : std::min(width, FLT_DIG),
  ...
  /*
    ...
    */
  have_space = (decpt <= 0 ? len - decpt + 2 : decpt > 0 && decpt < len ? len + 1 : decpt) <= width;
  /*
...
double mydtoa::strtod(const char *str, char **end, int *error)
{
  char buf[DTOA_BUFF_SIZE];
  double res;
  assert(end != NULL && ((str != NULL && *end != NULL) || (str == NULL && *end == NULL)) &&
         error != NULL);

  res = strtod_int(str, end, error, buf, sizeof(buf));
  return (*error == 0) ? res : (res < 0 ? -DBL_MAX : DBL_MAX);
}

double mydtoa::atof(const char *nptr)
{
  int error;
  const char *end = nptr + 65535; /* Should be enough */
  return (strtod(nptr, (char **)&end, &error));
}
...
static double strtod_int(const char *s00, char **se, int *error, char *buf, size_t buf_size)
{
  int scale;
    ...
    if (i == 0)
    {
      ...
      if (dsign)
      {
        if ((word0(&rv) & Bndry_mask1) == Bndry_mask1 &&
            word1(&rv) ==
                ((scale && (y = word0(&rv) & Exp_mask) <= 2 * P * Exp_msk1) ? (0xffffffff & (0xffffffff << (2 * P + 1 - (y >> Exp_shift)))) : 0xffffffff))
        {

...
int internal_str2dec(const char *from, decimal_t *to, char **end, bool fixed)
{
  ...
  *end = (char *)endp;
  ...
  {
    int str_error;
    int64_t exponent = myitoa::strtoint64(endp + 1, (char **)&end_of_string,
                                          &str_error);

    if (end_of_string != endp + 1)
    {
...
int decimal2double(const decimal_t *from, double *to)
{
  ...
  *to = mydtoa::strtod(strbuf, &end, &error);
...
int double2decimal(double from, decimal_t *to)
{
  ...
  int res;
  end = buff + mydtoa::gcvt(from, GCVT_ARG_DOUBLE, (int)sizeof(buff) - 1, buff, NULL);
  res = string2decimal(buff, to, &end);
  return res;
}

static int uint64_2dec(uint64_t from, decimal_t *to)
{
  int intg1;
  int error = E_DEC_OK;
  uint64_t x = from;
...
int uint64_2decimal(uint64_t from, decimal_t *to)
{
  to->sign = 0;
  return uint64_2dec(from, to);
}

int int64_2decimal(int64_t from, decimal_t *to)
{
  if ((to->sign = from < 0))
    return uint64_2dec(-from, to);
  return uint64_2dec(from, to);
...
int decimal2uint64(decimal_t *from, uint64_t *to)
{
  dec1 *buf = from->buf;
  uint64_t x = 0;
  int intg, frac;
  ...
  for (intg = from->intg; intg > 0; intg -= DIG_PER_DEC1)
  {
    uint64_t y = x;
    x = x * DIG_BASE + *buf++;
    if (UNLIKELY(y > ((uint64_t)ULLONG_MAX / DIG_BASE) || x < y))
    {
      *to = ULLONG_MAX;
      return E_DEC_OVERFLOW;
    }
...
int decimal2int64(decimal_t *from, int64_t *to)
{
  dec1 *buf = from->buf;
  int64_t x = 0;
  int intg, frac;
...
int decimal2lldiv_t(const decimal_t *from, lldiv_t *to)
{
  ...
  if (int_part == 2)
    to->quot = ((int64_t)from->buf[0]) * DIG_BASE + from->buf[1];
  else if (int_part == 1)
    to->quot = from->buf[0];
  else
    to->quot = 0;
...
int double2lldiv_t(double nr, lldiv_t *lld)
{
  ...
  lld->quot = (int64_t)(nr > 0 ? floor(nr) : ceil(nr));
  /* Multiply reminder to 10^9 and store into "rem" */
  lld->rem = (int64_t)rint((nr - (double)lld->quot) * 1000000000);
  /*
...
*/
int decimal2bin(decimal_t *from, char *para_to, int precision, int frac)
{
  ...
  uint8_t *to = (uint8_t *)para_to;
  uint8_t *orig_to = (uint8_t *)to;
  ...
  if (fsize0 > fsize1)
  {
    uint8_t *to_end = orig_to + orig_fsize0 + orig_isize0;
...
int bin2decimal(const char *para_from, decimal_t *to, int precision, int scale)
{
  const uint8_t *from = (const uint8_t *)para_from;
  ...
  sanity(to);
  d_copy = (uint8_t *)alloca(bin_size);
  ...
  if (*buf1 >= DIG_BASE)
  {
    ...
    *buf1 -= DIG_BASE;
    ...
    if (UNLIKELY(carry))
    {
      ...
      for (buf1 = to->buf + intg0 + (std::max)(frac0, 0); buf1 > to->buf; buf1--)
      {
...
int decimal_result_size(decimal_t *from1, decimal_t *from2, char op, int param)
{
  ...
  {
  case '-':
    return ROUND_UP((std::max)(from1->intg, from2->intg)) +
           ROUND_UP((std::max)(from1->frac, from2->frac));
  case '+':
    return ROUND_UP((std::max)(from1->intg, from2->intg) + 1) +
           ROUND_UP((std::max)(from1->frac, from2->frac));
  case '*':
